...
def participant_attribute_collector(pac_pad):
    pac_result = {}
    for participant in pac_pad:
        for p_attribute in pac_pad[participant]["Participant_Attributes"]:
            if not p_attribute in pac_result:
                pac_result[p_attribute] = {}
            if not pac_pad[participant]["Participant_Attributes"][p_attribute] in pac_result[p_attribute]:
                pac_result[p_attribute][pac_pad[participant]["Participant_Attributes"][p_attribute]] = 0
    return pac_result
...
def combo_selection(c_s_pad, number_pools, pac):
    ...
    for i in range(1, number_pools + 1):
        ...
        participant_attribute_counter[i] = copy.deepcopy(pac)
...
def sample_attribute_collector(sac_pad):
    sac_result = {}
    for participant in sac_pad:
        for sample in sac_pad[participant]['Sample_IDs']:
            for s_attribute in sac_pad[participant]['Sample_IDs'][sample]:
                if s_attribute == 'Pool':
                    continue
                else:
                    if not s_attribute in sac_result:
                        sac_result[s_attribute] = {}
                    if not sac_pad[participant]['Sample_IDs'][sample][s_attribute] in sac_result[s_attribute]:
                        sac_result[s_attribute][sac_pad[participant]['Sample_IDs'][sample][s_attribute]] = 0
    return sac_result
...
def permutation_selection(ps_pad, perm_pool_count, sac):
    ...
    for i in range(1, perm_pool_count + 1):
        sample_attribut_counter[i] = copy.deepcopy(sac)
...
def main():
    ...
    s_attribute_collection = sample_attribute_collector(pad)
    permutation_selection(pad_selected_combo, pool_count, s_attribute_collection)
